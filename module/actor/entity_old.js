...
export default class ActorT20 extends Actor {
  ...
  /* -------------------------------------------- */
	...
	prepareDerivedData() {
		...
		// Prepare skill bonus./foundryvtt-v10/
		...
		data.attributes.carga = this._computeEncumbrance();
	}
	...
	*/
	...
	getRollData() {
		...
		const poderesTormenta = this.items.filter( p => p.type == "poder" &&
			( (p.system.tipo == "geral" && p.system.subtipo == "Tormenta") || ["Deformidade", "Linhagem Rubra Aprimorada", "Linhagem Rubra Superior"].includes(p.name) ) );
	...
	*/
	 ...
	 */
	 _prepareDefense(){
		...
		if ( !def.bonus ) data.attributes.defesa.bonus = 0;
		...
		for (let item of this.items ) {
			if (item.type == "equipamento" && item.system.equipado) {
				...
				let def = Number(item.system.armadura.value);
	...
	_calcPVPM() {
		...
		// const conabl = this.system.atributos.con.value;
	...
	async descanso(modificador=1, modPV=0, modPM=0, curaCP=false, toChat=true) {
		...
		const pv = this.system.attributes.pv.value;
		let rec = {
			pv:0,
			pm:0
		}
	...
	*/
	...
	_computeEncumbrance(actorData) {
		...
		if( rule == 'core' ){
			...
			const atrFor = this.system.atributos.for;
			const atrCrg = this.system.attributes.carga || {max:0, lev:0};
			...
			let coins = Object.values( this.system.dinheiro ).reduce((a, b) => a + b);
	...
	async _preCreate(data, options, user) {
		...
		// Token size category
		const size = CONFIG.T20.tokenSizes[this.system.tracos.tamanho || "med"];
	...
	async _preUpdate(changed, options, user) {
		...
		const newSize = foundry.utils.getProperty(changed, "tracos.tamanho");
	 ...
	 */
	 async rollPericia(key, options = {message: true}) {
		let pericia = foundry.utils.deepClone( this.system.pericias[key] );
		const actor = this;
		...
		if( needsConfiguration ){
			...
			let active = this.effects.filter(ef => ef.getFlag("tormenta20","onuse") && ef.getFlag("tormenta20","pericia") && !ef.disabled);
	...
	applyActiveEffects() {
		...
		const changes = this.effects.reduce((changes, e) => {
			if (e.disabled || e.flags?.tormenta20?.onuse) return changes;
			return changes.concat(e.changes.map(c => {
				c = duplicate(c);
				if (c.key.match(/(system.)(.*)(.condi|.outros|.bonus|.value)|data.modificadores/i) && c.mode === 2 && !c.value.toString().match(/^[+|-][\d+|@\w+]/i)) {
