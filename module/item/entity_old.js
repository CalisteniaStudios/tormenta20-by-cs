...
export default class ItemT20 extends Item {
	 ...
	 */
	 get hasAttack() {
		...
		!!(this.system.rolls?.ataque[0] && this.system.rolls.ataque[0].parts.length);
	}
	...
	*/
	prepareDerivedData() {
		...
		// console.warn('prepareDerivedData', this.system );
		const C = CONFIG.T20;
		const labels = this.labels = {};
		
		// Classes
		if ( data.type === "classe" ) {
			data.niveis = Math.clamped(data.niveis, 1, 20);
		}
		...
		if ( data.rolls?.find(r=> r.type == "dano") ) {
			...
			if ( dano.parts ) {
				labels.dano = Object.values(dano.parts).map(d => d[0]).join(" + ").replace(/\+ -/g, "- ");
				labels.damageTypes = Object.values(dano.parts).map(d => C.damageTypes[d[1]]).join(", ");
			}
		}
		...
		// console.warn('prepareDerivedData', data );
	...
	getDerivedDamageLabel() {
		...
		if ( !this.hasDamage || !itemData ) return [];
		if( !this.isOwned ){
			// return [];
			console.log(itemData.rolls?.find(r=>r.type=="dano")?.parts);
		}
		const rollData = this.getRollData();
		// const derivedDamage = itemData.rolls?.find(r=>r.type=="dano")?.parts?.map(
		const parts = Object.values( itemData.rolls?.find(r=>r.type=="dano")?.parts );
		const derivedDamage = parts?.map((damagePart) => ({
			formula: simplifyRollFormula(damagePart[0], rollData, { constantFirst: false }),
			damageType: damagePart[1],
		}));
		console.log(derivedDamage);
	...
	getAttackToHit() {
		...
		if ( !this.isOwned ) {
			let toHitLabel = '' + roll.parts[1][0];
			if( roll.parts[1][1] ){
				toHitLabel = toHitLabel + ' ['+ roll.parts[1][1] +']';
			}
	...
	async roll({configureDialog=true, rollMode, createMessage=true, extra={}}={}) {
		...
		if ( true ) {
			...
			// item.system.update({_id: this.id}); // Retain the original ID (needed until 0.8.2+)
   ...
   */
	...
	async _preCreate(data, options, user) {
		...
		if (updates) return this.update(updates);
	}
	...
	async _preUpdate(changed, options, user) {
		...
		// console.warn('_preUpdate', changed, options);
	}

	/** @inheritdoc */
	_onUpdate(changed, options, user){
		super._onUpdate(changed, options, user);
		// console.warn('_onUpdate', changed, options);
	...
	static async createScrollFromSpell(magia) {
		...
		// Get spell data
		const itemData = magia instanceof ItemT20 ? magia : magia;
		...
		// Get scroll data
		const scrollData = {"ownership":{"default":0},"type":"consumivel","system":{"peso":0,"qtd":1,"preco":0}};
		scrollData.img = "systems/tormenta20/icons/itens/pergaminho.webp";

		// Split the scroll description into an intro paragraph and the remaining details
		const scrollDescription = scrollData.description;
		const pdel = '</p>';
